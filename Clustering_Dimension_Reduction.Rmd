...
```{r}
iris.pca <- data.matrix(iris[,-5])

center <- apply(iris.pca, 2, function(x) x - mean(x))

covariance <- cov(center)

eigenvalue <- eigen(covariance)$value

eigenvector <- eigen(covariance)$vector

PC <- as.data.frame(data.matrix(center) %*% eigenvector)
